...
### 11. 스텝 실행 간격 조정 모드 // 적용 완료 고속 진행 모드는 제외
- **난이도**: ⭐⭐⭐⭐
- **위험성**: 🟡 (중간) *모듈화 적용 시*
- **주요 작업**:
  - 시간 스텝 모드: 고정 시간 간격
  - 이벤트 스텝 모드: 이벤트 완료 후 스텝 // 해당 기능은 필요 없음
  - ~~엔진 코어 로직 수정~~ → 모드별 독립 모듈 추가
  - 전체 실행 버튼 이름을 스텝 연속 실행으로 변경
  - 고속 진행 모드 추가: 프론트엔드에 정보가 실시간 업데이트 되지 않고 현재 설정되어 있는 환경에 따라 백엔드에서만 시뮬레이션이 실행되고 마지막 결과만 프론트 엔드에 업데이트(지정 된 배출 수량 혹은 시간만큼 백엔드에서만 시뮬레이션 진행, 브레이크 포인트도 무시하고 진행함, 스텝 실행 시에는 효과가 없을 것으로 보이지만 전체 실행에서는 큰 효과를 볼 수 있을것으로 기대됨)
